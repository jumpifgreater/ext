...
inline void RenderESP(ImDrawList* drawList)
{
    ...
    auto cameraPos = mem->read<Vectors::Vector3>(Globals::Rblx::Camera.addr + offsets::CameraPos);
    ...
    for (auto& player : Globals::Caches::CachedPlayerObjects)
    {
        ...
        if (player.Team.addr == localTeam.addr && Options::ESP::TeamCheck)
            continue;
        ...
        Roblox torso(0), rightLeg(0), leftLeg(0), rightArm(0), leftArm(0);
        if (rigType == 0) {
            torso = player.Torso;
            ...
            torso = player.UpperTorso;
            rightLeg = player.RightFoot;
            leftLeg = player.LeftFoot;
            rightArm = player.RightHand;
            leftArm = player.LeftHand;
        }
        ...
        if (Options::ESP::ESP3D)
        {
            ...
            auto head = player.Head;
            auto leftLeg = (player.RigType == 0) ? player.LeftLeg : player.LeftFoot;
