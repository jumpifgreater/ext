...
inline Vectors::Vector3 GetTargetPosition(RobloxPlayer player)
{
    ...
    case 2: // Left Arm
        if (player.RigType == 0) // R6
            return player.LeftArm.Position();
        else // R15
            return player.LeftHand.Position();
    case 3: // Right Arm
        if (player.RigType == 0) // R6
            return player.RightArm.Position();
        else // R15
            return player.RightHand.Position();
    case 4: // Left Leg
        if (player.RigType == 0) // R6
            return player.LeftLeg.Position();
        else // R15
            return player.LeftFoot.Position();
    case 5: // Right Leg
        if (player.RigType == 0) // R6
            return player.RightLeg.Position();
        else // R15
            return player.RightFoot.Position();
    default:
...
inline RobloxPlayer GetClosestPlayer()
{
    ...
    auto localTeam = Globals::Rblx::LocalPlayer.Team();
    auto localCharacter = Globals::Rblx::LocalPlayer.Character();
    ...
    for (auto& player : Globals::Caches::CachedPlayerObjects)
    {
        ...
        if (player.addr == Globals::Rblx::LocalPlayer.addr)
            continue;

        if (player.Team.addr == localTeam.addr && Options::Aimbot::TeamCheck)
            continue;
...
{
    Matrixes::Matrix3x3 currentRotation = mem->read<Matrixes::Matrix3x3>(Globals::Rblx::Camera.addr + offsets::CameraRotation);

    sCFrame cameraCFrame = Globals::Rblx::Camera.CFrame();
    Vectors::Vector3 camPos = mem->read<Vectors::Vector3>(Globals::Rblx::Camera.addr + offsets::CameraPos);
    ...
    mem->write<Matrixes::Matrix3x3>(Globals::Rblx::Camera.addr + offsets::CameraRotation, smoothedMatrix);
}
...
inline void RunAimbot(ImDrawList* drawList)
{
    ...
    auto localCharacter = Globals::Rblx::LocalPlayer.Character();
    auto localHRP = localCharacter.FindFirstChild("HumanoidRootPart");
    auto Dimensions = mem->read<Vectors::Vector2>(Globals::Rblx::VisualEngine + offsets::Dimensions);
    ...
    {
        if (Options::Aimbot::CurrentTarget.addr == 0 ||
            Options::Aimbot::CurrentTarget.Health == 0 ||
            (Options::Aimbot::CurrentTarget.Health <= 4 && Options::Aimbot::DownedCheck) ||
            (Options::Aimbot::CurrentTarget.Team.addr == localTeam.addr && Options::Aimbot::TeamCheck) ||
    ...
    auto sensitivity = mem->read<float>(mem->getBase() + offsets::MouseSensitivity);
