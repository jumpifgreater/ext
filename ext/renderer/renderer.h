...
#include "../impl/esp/esp.h"
#include "../impl/aimbot/aimbot.h"
#include "W2S.h"
...
void ShowImgui()
{
    ...
    while (!done)
    {
        ...
        auto character = Globals::Rblx::LocalPlayer.Character();

        if (showMenu)
        {
            ImGui::Begin("@niggerkiller", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings);
        ...
        if (IsGameOnTop("Roblox"))
        {
            ...
            std::string str = "@niggerkiller | " + std::to_string(static_cast<int>(io.Framerate)) + " FPS";
        ...
        if (showMenu && explorer)
        {
            ...
            for (auto& child : Globals::Rblx::DataModel.GetChildren())
            {
                DrawNode(child);
            }
            ImGui::End();
        ...
        if (playerList && showMenu)
        {
            ...
            for (auto& player : Globals::Caches::CachedPlayerObjects)
            {
                ...
                if (ImGui::Button(teleportID.c_str()))
                {
                    ...
                    auto primitive = mem->read<uintptr_t>(character.FindFirstChild("HumanoidRootPart").addr + offsets::Primitive);

                    std::thread([=]() {
                        for (auto i = 0; i != 100; i++)
                            mem->write<Vectors::Vector3>(primitive + offsets::Position, { localCFrame.x, localCFrame.y, localCFrame.z });
                ...
                if (ImGui::Button(spectateID.c_str()))
                {
                    ...
                    {
                        spectatingSubject = "";
                        mem->write<uintptr_t>(Globals::Rblx::Camera.addr + offsets::CameraSubject, character.FindFirstChildWhichIsA("Humanoid").addr);
                    }
                    else
                    {
                        mem->write<uintptr_t>(Globals::Rblx::Camera.addr + offsets::CameraSubject, player.Humanoid.addr);
